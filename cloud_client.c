...
static void load_schedules(void)
{
    ...
    if (verr == ESP_OK && version == 2 && nerr == ESP_OK && n <= SCHEDULE_MAX && n > 0) {
        ...
        ESP_LOGW(TAG, "Schedule cache is invalid; ignoring corrupted cache");
        memset(schedules, 0, sizeof(schedules));
        schedule_count = 0;
...
void cloud_client_set_credentials(const char *base_url, const char *device_id, const char *device_token)
{
    ...
    bool need_start = have_creds && !g_task_started;
    if (need_start) g_task_started = true;
    xSemaphoreGive(cloud_mutex);
    if (!have_creds) g_online = false;
    if (need_start) xTaskCreate(cloud_task,"cloud_client",6144,NULL,3,NULL);
}
