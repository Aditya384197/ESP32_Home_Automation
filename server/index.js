...
async function api(env,req,url){
  ...
  const pr=p==='/api/device/poll';if(pr&&req.method==='POST'){
    const d=await deviceToken(env,req);if(!d)return json({error:'unauthorized'},401);const b=await body(req);if(b?.deviceId!==d.id)return json({error:'device id mismatch'},403);const states=Array.isArray(b.states)?b.states.slice(0,5).map(x=>x?1:0):[0,0,0,0,0];const enabled=Array.isArray(b.enabled)?b.enabled.slice(0,5).map(Boolean):[true,true,true,false,false];await env.DB.prepare('UPDATE devices SET online=1,last_seen=?,states=?,enabled=? WHERE id=?').bind(now(),JSON.stringify(states),JSON.stringify(enabled),d.id).run();const cmds=await env.DB.prepare('SELECT id,relay,state FROM commands WHERE device_id=? ORDER BY id LIMIT 64').bind(d.id).all();if(cmds.results?.length)await env.DB.prepare(`DELETE FROM commands WHERE id IN (${cmds.results.map(()=>'?').join(',')})`).bind(...cmds.results.map(x=>x.id)).run();const sch=await env.DB.prepare('SELECT id,relay,hour,minute,action,days,enabled,duration_minutes AS durationMinutes FROM schedules WHERE device_id=? ORDER BY id LIMIT 64').bind(d.id).all();return json({commands:cmds.results||[],schedules:sch.results||[]});
  }
  return null;
}
